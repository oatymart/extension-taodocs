...
// Platform themes
use oat\taoDocs\model\theme\TaodocsDefaultTheme;
...
foreach($itemThemes as $themeName => $isDefault){
    ...
    ThemeRegistry::getRegistry()->registerTheme(
        $themeId,
        $themeName,
        implode(DIRECTORY_SEPARATOR, array('taoDocs', 'views', 'css', 'themes', 'items', $pathFragment, 'theme.css')), array('items')
    );
}
...
$themeService->setTheme(new TaodocsDefaultTheme());
